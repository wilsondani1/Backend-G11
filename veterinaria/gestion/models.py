...
class ManejoUsuario(BaseUserManager):
    def create_superuser(self,correo,nombre,apellido,password,tipoUsuario):
        if not correo:
            raise ValueError('El debe tener correo')
...
class Usuario(AbstractBaseUser,PermissionsMixin):
    ...
    apellido = models.TimeField(null=False)
    correo = models.EmailField(max_length=100,unique=True,null=False)
    password = models.TextField(null=False)
    tipousuario = models.CharField(max_length=100,choices=[('ADMIN','ADMIN'),('CLIENTE','CLIENTE')],db_column='tipo_usuario')
